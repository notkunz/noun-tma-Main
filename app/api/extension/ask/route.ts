...
export async function POST(req: Request) {
  try {
    ...
    const { question, session_id, course_id, detected_course_code, options } =
      await req.json();
    ...
    if (session.question_count >= 10)
      return NextResponse.json(
        { error: "TMA limit of 10 questions reached" },
        { status: 400, headers: corsHeaders },
      );
    ...
    if (detected_course_code) {
      ...
      const sessionCode = sessionCourse?.course_code
        ?.replace(/\s+/g, "")
        .toUpperCase();
...
- Otherwise reply NO_MATCH`,
    ...
    if (bankHit) {
      ...
      const { data: courseData } = (await supabaseAdmin
        .from("courses")
        .select(
          "course_title, course_code, material_text, shared_material_code",
        )
        .eq("id", course_id)
        .single()) as { data: any };
...
8. If not found reply: ANSWER_NOT_FOUND`,
      ...
      answerText =
        result.choices[0]?.message?.content || "Could not generate answer.";
