...
export async function POST(req: Request) {
  try {
    ...
    const { data: material } = (await supabaseAdmin
      ...
      .single()) as { data: SharedMaterialRow | null };

    if (!material?.material_url) {
      return NextResponse.json(
        { error: "No PDF uploaded for this course code" },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Failed to read PDF: " + getErrorMessage(err) },
        { status: 500 },
      );
    }
    ...
    if (!extractedText || extractedText.trim().length < 50) {
      return NextResponse.json(
        { error: "PDF appears image-based or empty." },
        { status: 400 },
      );
    }
